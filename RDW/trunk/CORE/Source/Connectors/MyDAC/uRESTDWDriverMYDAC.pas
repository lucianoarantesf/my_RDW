unit uRESTDWDriverMYDAC;

interface

uses SysUtils,  Classes,   DB, MyClasses, MyAccess,  MyScript,    DADump,
     MyDump,   VirtualTable,  MemDS, DBAccess,
     uDWConsts, uDWConstsData, uRESTDWPoolerDB,   uDWJSONInterface,
     Variants,  uDWJSONObject, uDWJSONTools,      uDWMassiveBuffer,
     uDWDatamodule,            SysTypes,          uSystemEvents,
     {$IFDEF FPC}uDWConstsCharset,{$ENDIF}uDWDataset;

Type

 TSQLToBin = class(TComponent)
 public
    procedure SetParamsAndValue(Source : TDataSet; Params: TDAParams);
//    Procedure SaveToStream(Sql : TUniSQL; Stream : TMemoryStream); overload;
    Procedure SaveToStream(Query : TMyQuery; Stream : TMemoryStream); overload;
//    procedure LoadFromStream(Stream : TMemoryStream; sql : TUniSQL); overload;
    procedure LoadFromStream(Stream : TMemoryStream; Query : TMyQuery); overload;
 end;



 TRESTDWDriverMYDAC   = Class(TRESTDWDriver)
 Private
  vFDConnection                 : TMyConnection;
...
Function TRESTDWDriverMYDAC.ProcessMassiveSQLCache(MassiveSQLCache      : String;
                                                    Var Error            : Boolean;
...
Var
 ...
 Begin
  ...
  For I := 0 To Params.Count -1 Do
   ...
   End;
 End;
 Function LoadMassive(Massive : String; Var Query : TMyQuery) : Boolean;
 Var
  X, A, I         : Integer;
   ...
   For X := 0 To bJsonArray.ElementCount -1 Do
    Begin
     ...
     Try
      ...
      begin
        MemDS   := Decodeb64Stream(TDWJSONObject(bJsonValueB).pairs[2].Value);
...
Begin
 ...
 Try
  Error      := False;
  vTempQuery := TMyQuery.Create(Owner);
...
Function TRESTDWDriverMYDAC.ApplyUpdates_MassiveCache(MassiveCache          : String;
                                                       Var Error             : Boolean;
                                                       Var MessageError      : String) : TJSONValue;
Var
 vTempQuery        : TMyQuery;
...
//  bJsonArray     : udwjson.TJsonArray;
  ...
  bJsonValue     : TDWJSONObject;
  bJsonArray     : TDWJSONArray;
  Procedure PrepareData(Var Query      : TMyQuery;
                        MassiveDataset : TMassiveDatasetBuffer;
                        Var vError     : Boolean;
                        Var ErrorMSG   : String);
  Var
...
Procedure TRESTDWDriverMYDAC.Close;
Begin
 {$IFNDEF FPC}Inherited;{$ENDIF}
...
Function TRESTDWDriverMYDAC.ConnectionSet: Boolean;
Begin
 Result := vFDConnection <> Nil;
End;

Class Procedure TRESTDWDriverMYDAC.CreateConnection(Const ConnectionDefs : TConnectionDefs;
...
function TRESTDWDriverMYDAC.ExecuteCommand(SQL                  : String;
                                            Params               : TDWParams;
                                            Var Error            : Boolean;
                                            Var MessageError     : String;
                                            Var BinaryBlob       : TMemoryStream;
  ...
  Begin
   ...
   Else
    Begin
     For I := 0 To Params.Count -1 Do
      begin
       With TDAParam(vTempQuery.Params.Add) do
        Begin
         vParamName := Copy(StringReplace(Params[I].ParamName, ',', '', []), 1, Length(Params[I].ParamName));
         Name := vParamName;
         ParamType := ptInput;
...
procedure TRESTDWDriverMYDAC.ExecuteProcedure(ProcName         : String;
                                           Params           : TDWParams;
                                           Var Error        : Boolean;
                                           Var MessageError : String);
Var
...
Begin
 {$IFNDEF FPC}Inherited;{$ENDIF}
 Error  := False;
 vTempStoredProc                               := TMyStoredProc.Create(Owner);
 Try
...
//  vTempStoredProc.FormatOptions.StrsTrim2Len   := StrsTrim2Len;
  ...
  vFDConnection.Commit; {CommitRetaining};
 Except
  On E : Exception do
   Begin
    Try
...
procedure TRESTDWDriverMYDAC.ExecuteProcedurePure(ProcName         : String;
                                               Var Error        : Boolean;
                                               Var MessageError : String);
Var
 vTempStoredProc : TMyStoredProc;
...
Function TRESTDWDriverMYDAC.ApplyUpdates(Massive,
                                          SQL               : String;
                                          Params            : TDWParams;
                                          Var Error         : Boolean;
                                          Var MessageError  : String;
...
Function  TRESTDWDriverMYDAC.GetGenID(Query   : TComponent;
                                       GenName : String): Integer;
Begin
 Result := -1;
End;

Function TRESTDWDriverMYDAC.ApplyUpdatesTB(Massive              : String;
                                            Params               : TDWParams;
                                            Var Error            : Boolean;
                                            Var MessageError     : String;
                                            Var RowsAffected     : Integer) : TJSONValue;
...
// vZSequence     : TZSequence;
 ...
 Begin
  ...
  //                                      vFieldChanged := StreamToHex(vStringStream) <> MassiveField.Value;
   ...
   End;
 End;
 Function LoadMassive(Massive : String; Var Query : TMyTable) : Boolean;
 Var
  MassiveDataset : TMassiveDatasetBuffer;
  A, B           : Integer;
  Procedure PrepareData(Var Query      : TMyTable;
                        MassiveDataset : TMassiveDatasetBuffer;
                        Var vError     : Boolean;
                        Var ErrorMSG   : String);
  Var
...
Begin
 ...
 Try
  ...
  Error          := False;
  vStringStream  := Nil;
  vTempQuery     := TMyTable.Create(Owner);
...
Function TRESTDWDriverMYDAC.ExecuteCommandTB(Tablename            : String;
                                              Params               : TDWParams;
                                              Var Error            : Boolean;
                                              Var MessageError     : String;
                                              Var BinaryBlob       : TMemoryStream;
...
Function TRESTDWDriverMYDAC.GetConnection: TMyConnection;
Begin
 Result := vFDConnection;
End;

Function TRESTDWDriverMYDAC.InsertMySQLReturnID(SQL              : String;
                                             Params           : TDWParams;
                                             Var Error        : Boolean;
...
Begin
 ...
 Error  := False;
 vStringStream := Nil;
 fdCommand := TMyScript.Create(Owner);
 Try
...
procedure TRESTDWDriverMYDAC.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
Procedure TRESTDWDriverMYDAC.GetTableNames(Var TableNames       : TStringList;
                                            Var Error            : Boolean;
                                            Var MessageError     : String);
Begin
...
Procedure TRESTDWDriverMYDAC.GetProcNames(Var ProcNames        : TStringList;
                                           Var Error            : Boolean;
                                           Var MessageError     : String);
Begin
...
Procedure TRESTDWDriverMYDAC.GetProcParams(ProcName             : String;
                                            Var ParamNames       : TStringList;
                                            Var Error            : Boolean;
                                            Var MessageError     : String);
Var
 vFDStoredProc : TMyStoredProc;
 I             : Integer;
Begin
 ParamNames := TStringList.Create;
 vFDStoredProc := TMyStoredProc.Create(Nil);
 Try
  If Not vFDConnection.Connected Then
   vFDConnection.Connected := True;
...
Procedure TRESTDWDriverMYDAC.GetKeyFieldNames(TableName            : String;
                                               Var FieldNames       : TStringList;
                                               Var Error            : Boolean;
                                               Var MessageError     : String);
Var
...
Begin
 ...
 Try
  ...
  Begin
   If vSchema <> '' Then
    vFDConnection.Database := vSchema;
   vFDConnection.Connected := True;
  End;
...
Procedure TRESTDWDriverMYDAC.GetFieldNames(TableName            : String;
                                            Var FieldNames       : TStringList;
                                            Var Error            : Boolean;
                                            Var MessageError     : String);
Var
...
Function TRESTDWDriverMYDAC.OpenDatasets   (DatasetsLine     : String;
                                             Var Error        : Boolean;
                                             Var MessageError : String;
...
Begin
 ...
 bJsonArray      := Nil;
 vStream         := Nil;
 vTempQuery      := TMyQuery.Create(Nil);
 Try
  If Not vFDConnection.Connected Then
   vFDConnection.Connected := True;
  vTempQuery.Connection   := vFDConnection;
...
Procedure TRESTDWDriverMYDAC.PrepareConnection(Var ConnectionDefs : TConnectionDefs);
Begin
 {$IFNDEF FPC}Inherited;{$ENDIF}
End;
...
Procedure TRESTDWDriverMYDAC.SetConnection(Value: TMyConnection);
Begin
...
procedure TSQLToBin.LoadFromStream(Stream: TMemoryStream; Query: TMyQuery);
var
  LocalSql : TMyQuery;
begin
    try
      LocalSql := TMyQuery.Create(nil);
      Stream.Position := 0;
      Stream.ReadComponent(LocalSql);
...
procedure TSQLToBin.SaveToStream(Query: TMyQuery; Stream: TMemoryStream);
var
  LocalSql : TMyQuery;
begin
    try
      ...
      LocalSql.Name := 'MyQuery'+IntToStr(Random(1000));
      Stream.Position := 0;
      Stream.WriteComponent(LocalSql);
    finally
      LocalSql.Free;
...
procedure TSQLToBin.SetParamsAndValue(Source: TDataSet; Params: TDAParams);
var
  i : integer;
  Field : TField;
begin
