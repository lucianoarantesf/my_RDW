{
 ...
 Desenvolvedor : Gilberto Rocha da Silva
 Data : 27/10/2019
}

unit uDWUpdSqlEditor;

interface

uses
  SysUtils, Dialogs, Forms, ExtCtrls, StdCtrls, ComCtrls, DBGrids, uRESTDWPoolerDB, DB{$IFNDEF FPC}, Grids{$ENDIF}, Controls,
  Classes,{$IFDEF FPC}ComponentEditors, FormEditingIntf, PropEdits, lazideintf{$ELSE}DesignEditors, DesignIntf{$ENDIF}, uDWConstsData;
...
Procedure TFrmDWUpdSqlEditor.SetClientSQL(Value: TRESTDWClientSQLBase);
Var
 vMemString : TStringList;
Begin
    ...
    Begin
     If RESTDWStoredProc.DataBase <> Nil Then
      Begin
       ...
       RESTDWDatabase.EncodeStrings         := RESTDWStoredProc.DataBase.EncodeStrings;
