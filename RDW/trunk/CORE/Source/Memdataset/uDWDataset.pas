...
Uses
 ...
 {$ELSE}{$IF CompilerVersion <= 22}
        ...
        {$IFEND}
 {$ENDIF}
 SysUtils, Classes, uDWConstsCharset, uZlibLaz;
...
Type
 ...
 TDWBRSourceSide   = (dwDelphi, dwLazarus);
...
Type
 ...
 Type
  TDWDbTableHeader = Packed record
   VersionNumber,
   RecordCount,
   CheckRecCount,
...
End;
  ...
  End;
  TDWTableStorageClass = class of TDWTableStorage;
...
{$IFDEF REGION}{$ENDREGION}{$ENDIF}
  ...
  End;
  {$ENDIF}
  TDWMemtableOptionD  = (dwPersistentData, dwStored, dwSetEmptyStrToNull, dwSkipUnSupportedFieldTypes);
  TDWMemtableOptionsD = Set Of TDWMemtableOptionD;
  ...
  Public
   ...
   Property DataByName[Const Name                   : String]  : Variant Read GetDataByName       Write SetDataByName; Default;
   Property Options         : TDWMemtableOptionsD                        Read FOptions            Write FOptions;
...
Resourcestring
 ...
 Var
  {$IFDEF SUPPORTS_CLASS_FIELDS}
  vTableStorage : TDWTableStorageClass = TDWTableStorage;
  {$ELSE}
  vTableStorage : TDWTableStorage = nil;
  {$ENDIF}
...
Begin
 ...
 Stream.ReadBuffer(TableHeader, Sizeof(TDWDbTableHeader));
 //canceled version
 ...
 For I := 0 To RecordCount - 1 Do
  Begin
   ...
   If Assigned(OnWriterProcess) Then
    OnWriterProcess(TDWCustomDataSet(FTable), I +1, RecordCount, AbortProcess);
...
Class function TDWTableStorage.GetDbResource(Const Name : string): TStream;
Begin
 Result := nil;
End;

Class procedure TDWTableStorage.GetTimeZoneBias;
Begin
 FStaticBias := 0.0;
End;

Class Function TDWTableStorage.OutputDirectory     : String;
Begin
 Result := ExtractFilePath(ParamStr(0));
End;

Class Function TDWTableStorage.GetDefaultExt       : String;
Begin
 Result := '.dwtb';
End;

Class function TDWTableStorage.GetHeaderVersion: Integer;
Begin
 Result := HeaderVersion;
End;

Class Function TDWTableStorage.OutputFileName(Const FileName : String) : String;
Begin
...
Class Function TDWTableStorage.IsSystemData(DataSet   : TDataSet;
...
{$IF Defined(HAS_FMX)}
{$IF Defined(HAS_UTF8)}
Class Function TDWTableStorage.GetTableDefs(DataSet: TDataSet) : String;
{$ELSE}
Class Function TDWTableStorage.GetTableDefs(DataSet: TDataSet) : AnsiString;
{$IFEND}
{$ELSE}
Class Function TDWTableStorage.GetTableDefs(DataSet: TDataSet) : AnsiString;
{$IFEND}
...
Class Function TDWTableStorage.GetProductInfo: Int64;
Var
...
Class procedure TDWTableStorage.LoadArray(Stream     : TStream;
...
Class Procedure TDWTableStorage.LoadTable(Const TableName : String;
                                          Var   VArray    : TVariantArray);
Var
...
Class Procedure TDWTableStorage.SaveTable(Const TableName : String;
                                          Var   VArray    : TVariantArray);
Var
 Stream : TMemoryStream;
Begin
...
Class Function TDWTableStorage.SupportTimeStampOffset: Boolean;
Begin
 Result := True;
End;

Class Function TDWTableStorage.CheckForName(Const AName : String) : Boolean;
Const
 cIllegalChar = '=()+-*/%;:?''';
...
Class Function TDWTableStorage.GetColorIdentsStrings : TStrings;
Var
 I : Integer;
Begin
...
begin
  if DataSet is TDWCustomDataSet then
   Result := dwPersistentData in TDWCustomDataSet(DataSet).Options
  else
    Result := False;
end;
...
procedure TDWCustomDataSet.Loaded;
Begin
 Inherited Loaded;
 Try
...
Procedure TDWCustomDataSet.SaveToStream(F : TStream);
Begin
 FArray.SaveToStream(F);
End;

Procedure TDWMemtable.Clear;
Begin
...
Procedure TDWMemtable.DeleteFields;
Begin
 Clear;
 FieldDefs.Clear;
 Fields.Clear;
...
Procedure TDWMemtable.Assign       (Source     : TDataSet;
...
Procedure TDWCustomDataSet.SaveDataToStream(F: TStream; SaveData: Boolean);
Var
...
Procedure TDWCustomDataSet.RaiseError(Fmt: String; Args: array of const);
Begin
 Raise DwDatabaseError.CreateFmt(Fmt, Args);
End;

Procedure TDWCustomDataSet.LoadDataFromStream(F : TStream);
Var
 Size      : Integer;
 OwnerData : OleVariant;
 P         : Pointer;
...
Procedure TDWCustomDataSet.SetEncoding(bValue : TEncodeSelect);
Begin
 vEncoding := bValue;
End;

Procedure TDWCustomDataSet.WriteMarker(F: TStream; Marker: Integer);
Begin
 Writeinteger(F, Marker);
End;

procedure TDWCustomDataSet.CheckMarker(F: TStream; Marker: Integer);
Var
 I, P : Integer;
Begin
 P := F.Position;
...
Procedure TDWCustomDataSet.ReadFieldDefsFromStream(F : TStream);
Var
 FS,
 I, ACount : Integer;
 FN        : String;
...
Procedure TDWCustomDataSet.SaveFieldDefsToStream   (F : TStream);
Var
 I  : Integer;
 FD : TFieldDef;
Begin
...
{$IFDEF FPC}
Procedure TDWCustomDataSet.SetDatabaseCharSet(Value : TDatabaseCharSet);
Begin
 vDatabaseCharSet       := Value;
...
Procedure TDWCustomDataSet.Done;
Begin
 FArray.Done;
End;
...
Initialization
 {$IFNDEF SUPPORTS_CLASS_FIELDS}
  vTableStorage := TDWTableStorage.Create;
 {$ENDIF}
